...
def _build_output_filename(
    ...
    return _build_safe_output_filename(parts, fallback_stem='llama-server')
...
def _worker_server_and_llama_benchy(
    ...
    try:
        server_exe = _find_llama_server_binary(install_dir)
        if server_exe:
            server_cmd = [server_exe]
            if server_model:
                server_cmd.extend(['-m', server_model])
            server_cmd.extend(all_server_options)
            if verbose:
                server_cmd.append('-v')
            logger.info("Running llama-server command: %s", ' '.join(server_cmd))
